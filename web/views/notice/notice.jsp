	...
	<div class="total">
		...
		<%-- <div class="right">
			...
			<div class="margin-list-head">
				<h2 style="color:#767676;"align="left">공지사항</h2>
				<!-- 파라미터 값 저장  -->
			...
			<div class="right">
				...
				<div class="margin-list-head">
					<div>
						...
						<input type="hidden" id="searchParameterValue" value=<%=search%>>
						<input type="hidden" id="keywordParameterValue" value=<%=keyword%>>
						<input type="hidden" id="date1Value" value=<%=date1%>> <input
							type="hidden" id="date2Value" value=<%=date2%>>
						...
						<!-- 11111111111111111111111홍석코드 111111111111111111111111111111111111-->
						<!-- 				&nbsp; <label>분류</label> <select id="searchType" name="searchType">
					로그인한 사용자에 대한 권한과 부서등에 따라서 alert등으로 접근 금지를 설정 한다. -
					<option>전체</option>
					<option>공지사항</option>
                    ...
                    <option></option>
						...
						<div class="table-line">
							<div>
								<!-- 검색 이다 이말이야 -->
								<label style="font-weight:bold;"> 기 간 설 정</label>&nbsp; <input type="date" id="time1"> ~ <input 
						 type="date" id="time2">
						 <br><br>
								<select id="search" name="search"
									style="height: 24px; margin-left: 300px; position: relative; left: 410px;">
									<option value="all">전체 보기</option>
									<option value="title">제목</option>
									<option value="content">내용</option>
								</select>&nbsp; <input class="form-control" type="text" id="keyword"
									name="keywords" placeholder="검색어를 입력하세요"
									style="width: 200px; height: 18px; position: relative; left: 400px;"
									value=<%=keyword%>>
								<button style="position: relative; left: 400px; height: 24px; background-color:#017DD6; color:white; border:0;"
									type="button" onclick="search();">검색하기</button>
								...
								<div class="pagingArea" align="center">
									<button
										onclick="location.href='/semi/noticeListServlet?currentPage=1'+show(); ">
										<<</button>
									<%
										if (currentPage <= 1) {
									%>
									...
									%>
									<button
										onclick="location.href='/semi/noticeListServlet?currentPage=<%=currentPage - 1%>'+show(); ">
										<</button>
									<%
										}
									%>
									...
									%>
									<button
										onclick="location.href='/semi/noticeListServlet?currentPage=<%=p%>'+show(); ">
										<%=p%>
									</button>
									<%
										}
									...
									%>
									<button
										onclick="location.href='/semi/noticeListServlet?currentPage=<%=maxPage%>'+show(); ">
										>></button>
								</div>
							</div>
						</div>
